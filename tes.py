...
from utils.helper import load_json, extract_path
...
async def main() -> Optional[list]:
    ...
    async for session in get_db():
        ...
        try:
            query_images =  await db.find(table=ClientPreview, fetch="all", filter='or', limit=QUERY_PER_PAGE, **formatted_result)
            all_data =  await db.find(table=ClientPreview, fetch="all", filter='or', **formatted_result)
            if query_images:
    ...
    encoded_image = query_model.encode(converted_image, batch_size=2)
    query_image = query_model.encode(resized, batch_size=2)
    results = query_model.query(query, encoded, k=50)
    ranked_results = [
        {"path": fullpaths[item["corpus_id"]], "score": item["score"]}
        for item in results
    ]
